...
export const ConditionsLoopsVisual: React.FC<ConditionsLoopsVisualProps> = ({ isRTL, isDark = false }) => {
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              >
                <div>
                  ...
                  <ul className={`space-y-3 pt-2 text-xs sm:text-sm ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                ...
                <div className={`mt-4 pt-3 border-t ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
                  <div className={`p-3 rounded-xl text-[11px] font-mono overflow-x-auto shadow-inner border ${
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-11 h-11 rounded-xl flex items-center justify-center font-black shadow-md ${
                        isDark ? 'bg-amber-500/20 border border-amber-500/40 text-amber-400' : 'bg-amber-50 border border-amber-200 text-amber-600'
                      }`}>
          ...
          {activeTab === 'simulator' && (
            ...
            >
              ...
              <div className={`p-5 sm:p-6 rounded-2xl border backdrop-blur-xl flex flex-col justify-between text-start ${
                isDark ? 'bg-slate-900/90 border-slate-800 text-white shadow-xl' : 'bg-white border-slate-200/90 text-slate-900 shadow-md'
              }`}>
                <div>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className={`flex justify-between text-xs font-bold ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
                      <span>{isRTL ? 'الإيرادات الحالية (Revenue):' : 'Current Revenue:'}</span>
                      <span className={`font-mono text-sm ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>${revenue.toLocaleString()}</span>
                    </div>
                    <input 
                      type="range" 
                      min="1000" 
                  ...
                  <div className={`p-4 rounded-xl border transition-all ${
                    isTargetMet 
                      ...
                      : (isDark ? 'bg-amber-950/60 border-amber-800 text-amber-200' : 'bg-amber-50 border-amber-200 text-amber-900')
                  }`}>
                ...
                <div className={`mt-3 text-[11px] flex items-center justify-between border-t pt-2 ${
                  isDark ? 'text-slate-400 border-slate-800' : 'text-slate-500 border-slate-100'
                }`}>
                  <span>{isRTL ? 'المنطق البرمجي:' : 'Evaluated:'}</span>
                  <code className={`font-mono font-bold ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>revenue &gt;= 5000</code>
                </div>
              </div>
              ...
              <div className={`p-5 sm:p-6 rounded-2xl border backdrop-blur-xl flex flex-col justify-between text-start ${
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <Settings className={`w-5 h-5 ${isDark ? 'text-amber-400' : 'text-amber-600'}`} />
                      <h3 className={`font-bold text-base ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className={`p-1 rounded-lg cursor-pointer ${isDark ? 'hover:bg-slate-800 text-slate-400' : 'hover:bg-slate-200 text-slate-600'}`}
                        title="Reset"
                      >
                      ...
                      return (
                        <div 
                          ...
                          className={`p-2 rounded-xl border text-xs flex items-center justify-between transition-all ${
                            isActive 
                              ? 'bg-amber-500/20 border-amber-500 text-amber-900 font-bold scale-[1.02] shadow-sm' 
                              : isProcessed 
                                ? (isDark ? 'bg-slate-950/60 border-slate-800 text-slate-300' : 'bg-slate-50 border-slate-200 text-slate-700')
                                : (isDark ? 'bg-slate-950/40 border-slate-800/60 text-slate-400' : 'bg-white border-slate-100 text-slate-500')
                          }`}
                        >
                          <div className="flex items-center gap-2">
                            <span className={`w-5 h-5 rounded-md flex items-center justify-center font-mono text-[10px] font-bold ${
                              isActive ? 'bg-amber-600 text-white' : (isDark ? 'bg-slate-800 text-slate-300' : 'bg-slate-200 text-slate-700')
                            }`}>
                              {idx + 1}
                            </span>
                            <span className="font-semibold">{row.client}</span>
                            <span className="text-[10px] opacity-70">({row.region})</span>
                          </div>

                          <div className="flex items-center gap-2">
                            <span className="font-mono">${row.rev}</span>
