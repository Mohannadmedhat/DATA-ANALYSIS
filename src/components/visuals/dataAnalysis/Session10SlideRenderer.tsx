...
export const Session10SlideRenderer: React.FC<Session10SlideRendererProps> = ({
  ...
  if (slide.id === 1) {
    return (
      <HeroCoverVisual 
        sessionNumber="10"
        titlePrefix="PYTHON"
        titleHighlight="CONTROL FLOW"
        subtitle="Conditional Statements (if/elif/else) · Loops (for/while) · Loop Control"
        floatingBadges={[
          { icon: GitBranch, label: "If / Elif / Else", position: "top-left", borderColor: "border-blue-500/50", textColor: "text-blue-400" },
          { icon: RotateCw, label: "For & While Loops", position: "top-right", borderColor: "border-cyan-500/50", textColor: "text-cyan-400" },
          { icon: Sliders, label: "Break & Continue", position: "bottom-left", borderColor: "border-orange-500/50", textColor: "text-orange-400" },
          { icon: Terminal, label: "Logic & Functions", position: "bottom-right", borderColor: "border-purple-500/50", textColor: "text-purple-400" }
        ]}
        statsCards={[
          ...
          { val: "Live Demos", label: "Interactive Labs", sub: "Code Execution Engine" }
        ]}
        onStart={onNext}
      />
    );
  ...
  // SLIDE 14: OUTRO / KEEP CODING
  // =========================================================
  if (slide.id === 14 || slide.type === 'outro') {
    return (
      <ThankYouVisual 
        sessionNumber="10"
        nextSessionNote="Next Session: Python Data Structures — Lists, Tuples, Dictionaries &amp; Sets"
        nextSessionButtonText="Open Session 11: Python Data Structures"
        onRestart={() => onSelectSlide ? onSelectSlide(0) : onNext?.()}
        onNextSession={onSwitchSession ? () => onSwitchSession('session-11') : undefined}
      />
    );
  }
